...
```{r}
#1
diamonds %>% filter (price < 400 | price > 10000)
...
#4
any(diamonds$cut == "Fair" & diamonds$price > 3000)
all(diamonds$cut == "Ideal" & diamonds$price > 2000)

#5
diamonds %>% mutate (price_cat1 = if_else(price > 500, "high", "low"))
diamonds %>% mutate (price_cat2 = case_when(
  price < 500 ~ "low",
  (price < 1000 & price > 500) ~ "medium",
  price > 1000 ~ "high",
  .default = NA))
```
